...
/*****
 * This file is just to capture the problem properties like number of links states etc
 * NOT TO BE USED FOR BENCHMARK PURPOSES
 * *****/
private int regressionLinks=0;
private int regressionOptimizedLinks=0;
private int simulationLinks=0;
private int enactedBehaviourStates=0;
private int enactedTargetStates=0;
private int environmentStates=0;
private int targetStates=0;
private ArrayList<Integer> behaviourStates = new ArrayList<Integer>();
...
public BehaviourSimulatorMorpheus(List<Behaviour> availableBehaviours, TransitionSystem environment, Behaviour targetBehaviour) throws NullPointerException, IllegalArgumentException {
    ...
    public void compute()
    {
    	...
    	regressionLinks = links.size();
    	enactedBehaviourStates = system.keySet().size();
    	enactedTargetStates = target.keySet().size();
    	...
    	while(linkRemoved && !links.isEmpty())
    	{
    		...
    		linkRemoved = false;
    		iterations++;
    		//if(iterations==2){System.out.println("Links remaining: "+links.size());}
    		...
    		for(SimulationLink link:links)
    		{
    			...
    			{
    				toRemove.add(link);
    				ArrayList links = linksMap.get(fromState);
    				links.remove(link);
    				//linksMap.get(fromState).remove(link);
    				linkRemoved = true; 
    	...
    	simulationLinks = links.size();
    	if(checkSolution()){System.out.println("Solution Exists");}
    	else{System.out.println("Solution doesn't exist");}
    	System.out.println("TotalIterations: "+iterations);
    	System.out.println("TotalLinksChecked: "+counter);
	...
	public void compute1()
    {
    	...
    	regressionOptimizedLinks = links.size();
    	enactedBehaviourStates = system.keySet().size();
    	enactedTargetStates = target.keySet().size();
    	...
    	System.out.println("TotalLinksChecked: "+counter);
    	simulationLinks = links.size();
    	System.out.println("SimulationLinks: "+links.size());
    	//System.out.println(links);
    }
    	...
    	while(!toCheck.isEmpty())
    	{
    		...
    		for(SimulationLink link:toCheck)
    		{
    			...
    			if(!isNextStateInSimulation(fromState, targetState))
    			{
    				...
    				links.remove(link);
    				bad.add(link);
    				//toAdd.addAll(linktree.get(link));
    				/*
    				HashSet<ArrayList<Integer>> fromStates = system.get(link.getFromState()).getFromStates();
    				for(ArrayList<Integer> state:fromStates)
    				{
    					ArrayList<SimulationLink> l = linksMap.get(state);
    					toAdd.addAll(l);
    				}*/
	...
	public void compute3()
    {
    	...
    	System.out.println("TimesLinksChecked: "+counter);
    	System.out.println("Iterations: "+iterations);
    }
	...
	public void compute4()
    {
    	...
    	System.out.println("TimesLinksChecked4: "+counter);
    	simulationLinks = links.size();
    	System.out.println("SimulationLinks4: "+links.size());
    }	
	...
	private void addToLinkTree(	HashMap<SimulationLink, HashSet<SimulationLink>> linktree,	SimulationLink link) {
		//get from states
		HashSet<ArrayList<Integer>> fromSysStates = system.get(link.getFromState()).getFromStates();
		HashSet<ArrayList<Integer>> fromtarStates = target.get(link.getSimulatedState()).getFromStates();
		for(List<Integer> state:fromSysStates)
		{
		    ...
		    for(SimulationLink l:links)
		    {
		    	if(fromtarStates.contains(l.getSimulatedState()))
		    	{
		    		HashSet<SimulationLink> children = linktree.get(link);
		    		if(children==null)
		    		{
		    			children = new HashSet<SimulationLink>();
		    			linktree.put(link, children);
		    		}
		    		children.add(l);
		    	}
		    }
	...
	private boolean checkSolution() {
		Set<List<Integer>> states = target.keySet();
		for(List<Integer> state:states)
		{
			if(!targetStateInSim(state))
			{
				return false;
			}
		}
    ...
    private boolean targetStateInSim(List<Integer> state) {
		for(SimulationLink l:links)
		{
			if(l.to.equals(state)){return true;}
		}
		return false;
	}
	...
	private boolean isNextStateInSimulation(List<Integer> fromState,List<Integer> targetState) {
		...
		{
			// target is the only and the first behaviour in the synchronous product of the enacted target
	...
	public void buildEnactedBehaviour(HashMap<List<Integer>, StateLinks> target, TransitionSystem env) {
	...
	public void buildEnactedBehaviour(List<Behaviour> behaviours, TransitionSystem env) {
		/* *
        						 ...
        						 for(Integer bs:behToStates)
        						 {
        							// for(State es:envToStates)
        							 for(Integer es:envToStates)
        							 {
        								 ...
        								 // add the tostates to the systems
	...
	public int getRegressionLinks(){return regressionLinks;}
	public int getRegressionOptimizedLinks(){return regressionOptimizedLinks;}
	public int getSimulationLinks(){return simulationLinks;}
	public int getSystemStatesCount(){return enactedBehaviourStates;}
	public int getTargetStatesCount(){return enactedTargetStates;}
     ...
     * **
     * @author nitin
     * Class to represent the transitions internal to the enacted systems
     */
